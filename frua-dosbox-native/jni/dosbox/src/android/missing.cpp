...
extern DECLSPEC int	SDLCALL SDL_OpenAudio(SDL_AudioSpec *desired, SDL_AudioSpec *obtained){
	*obtained = *desired;
	// see android_iface.cpp
	android_mixer_callback_cpp = desired->callback;
	android_mixer_callback = android_mixer_callback_c;
	int bufSize = android_open_audio(android_mixer_callback, desired->freq, desired->channels, 1, desired->samples << 2);
	if (bufSize == 0)
		return -1;
