...
static Bitu Normal_Loop(void) {
	...
	while (1) {
		if (PIC_RunQueue()) {
			...
			if (dosbox_config.abort == 1) {
				dosbox_config.abort = 2;
				return 1;
			}
			if (ticksRemain>0) {
				TIMER_AddTick();
...
void DOSBOX_Init(void) {
	...
	Pint = secprop->Add_int("memsize", Property::Changeable::OnlyAtStart,dosbox_config.memsize);
	Pint->SetMinMax(1,63);
	Pint->Set_help(
	...
	Pint = secprop->Add_int("frameskip",Property::Changeable::Always,dosbox_config.frameskip);
	Pint->SetMinMax(0,10);
	...
	if (dosbox_config.cycles)
		Pmulti_remain->SetValue("fixed");
	else
		Pmulti_remain->SetValue("auto");
	Pstring->Set_values(cyclest);

	char	buf[10]=""; if (dosbox_config.cycles) sprintf(buf, "%d", dosbox_config.cycles);
