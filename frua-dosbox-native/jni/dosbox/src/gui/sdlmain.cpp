...
#include "../src/android/android_internal.h"
...
void GFX_ResetScreen(void) {
	...
	if(android_reset_screen()) return;
...
Bitu GFX_SetSize(Bitu width,Bitu height,Bitu flags,double scalex,double scaley,GFX_CallBack_t callback) {
	...
	if(android_set_video_mode(width, height, bpp)) return 0;
...
bool GFX_StartUpdate(Bit8u * & pixels,Bitu & pitch) {
	if(android_lock_surface(&pixels)) return false;
	pitch=dosbox_config.rowbytes;
	//pitch=loadf->width * 2;
	sdl.updating=true;
	return true;
...
void GFX_EndUpdate( const Bit16u *changedLines ) {
	...
	android_unlock_surface(startLine, endLine);
...
void GFX_Events() {
	...
	if (dosbox_config.pause) {
		PauseDOSBox(true);
		//loadf->eventType = SDL_FIRSTEVENT;
		return;
	}

	android_event_t event1;
	...
	while ((!stopPoll) && android_poll_event(&event1))
	{
		android_event_t *event = &event1;
		switch (event->event_type) {
			...
			{
				JOYSTICK_Move_X(0, event->x/256.0f);
				JOYSTICK_Move_Y(0, event->y/256.0f);
			}
				break;
			case SDL_JOYBUTTONDOWN:
			{
				JOYSTICK_Button(0, event->keycode, true);
			}
				break;
			...
			case SDL_KEYDOWN:
			{
				...
				bool down = (event->event_type == SDL_KEYDOWN);
