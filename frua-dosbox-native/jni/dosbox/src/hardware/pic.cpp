...
//extern bool	enableCycleHack;	//locnet, cycle hack, some program will benefit when under stress
...
bool PIC_RunQueue(void) {
	...
	if (pic_queue.next_entry) {
		...
		if (cycles<CPU_CycleLeft) {
			...
			//locnet, cycle hack
			if (GCC_UNLIKELY(dosbox_config.cycle_hack && eventCount)) {
				CPU_Cycles += 50;
				//CPU_Cycles = cycles;
				if (CPU_Cycles > CPU_CycleMax) CPU_Cycles = CPU_CycleMax;
			}
		}
